    ...
    <div className="space-y-5">
      {skills.map((s) => {
        ...
        return (
          <div
            key={s.name}
            className="p-5 rounded-2xl border border-white/50 bg-white/60 backdrop-blur shadow-sm hover:shadow-lg transition-shadow"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <span className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-600/15 via-purple-600/15 to-pink-600/15 border border-gray-200/70 flex items-center justify-center">
                  <Icon className="h-5 w-5 text-indigo-700" />
                </span>
                <div>
                  <p className="font-semibold text-gray-900">{s.name}</p>
                  {s.note ? <p className="text-sm text-gray-600 mt-0.5">{s.note}</p> : null}
                </div>
              </div>
